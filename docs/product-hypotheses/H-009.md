...
- Notes:
  ...
  - `2026-04-02`: local history ingestion now stores raw token-usage rows separately and surfaces ingest-time totals for `input_tokens`, `cached_input_tokens`, `output_tokens`, `reasoning_output_tokens`, and `total_tokens`, which makes the breakdown visible before it is collapsed into higher-level marts.
  - `2026-04-02`: a real local smoke over `~/.codex` found 8,012 token-count events, of which 7,987 had usable token breakdowns; the parser now exposes the exact spent totals instead of leaving them buried in JSONL.
