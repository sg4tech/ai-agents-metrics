...
All commands are invoked via `./tools/codex-metrics <command> [flags]`.
...
```bash
./tools/codex-metrics start-task --title "Add typed pipeline contracts" --task-type product
./tools/codex-metrics start-task --title "Write retro for CODEX-52" --task-type retro
```
...
```bash
./tools/codex-metrics continue-task --task-id 2026-04-08-001 --failure-reason validation_failed
```
...
```bash
./tools/codex-metrics finish-task --task-id 2026-04-08-001 --status success
./tools/codex-metrics finish-task --task-id 2026-04-08-001 --status fail --failure-reason unclear_task
```
...
```bash
# New goal
./tools/codex-metrics update --title "Improve CLI help" --task-type product --attempts-delta 1
# Update existing
./tools/codex-metrics update --task-id 2026-03-29-010 --status success --notes "Validated"
```
...
```bash
./tools/codex-metrics merge-tasks --keep-task-id 2026-04-01-001 --drop-task-id 2026-04-01-002
```
...
```bash
./tools/codex-metrics audit-history
```
...
```bash
./tools/codex-metrics audit-cost-coverage
```
...
```bash
./tools/codex-metrics compare-metrics-history
```
...
```bash
./tools/codex-metrics render-report
```
...
```bash
./tools/codex-metrics ingest-codex-history
```
...
```bash
./tools/codex-metrics normalize-codex-history
```
...
```bash
./tools/codex-metrics derive-codex-history
```
...
```bash
./tools/codex-metrics derive-retro-timeline
```
...
```bash
./tools/codex-metrics init
./tools/codex-metrics init --force --write-report
```
...
```bash
./tools/codex-metrics bootstrap --target-dir /path/to/repo --dry-run
./tools/codex-metrics bootstrap --target-dir /path/to/repo
```
...
```bash
./tools/codex-metrics install-self
./tools/codex-metrics install-self --copy
```
...
```bash
./tools/codex-metrics completion zsh >> ~/.zshrc
./tools/codex-metrics completion bash >> ~/.bashrc
```
...
```bash
./tools/codex-metrics ensure-active-task
```
...
```bash
./tools/codex-metrics verify-public-boundary --repo-root /path/to/public-clone
```
