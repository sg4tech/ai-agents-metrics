...
- When a layer is described as `public`, `protected`, `private`, `internal`, or otherwise sanctioned, encode that boundary in module structure instead of leaving it as convention only. If a runtime surface is meant to be the one supported entrypoint, give it an explicit module or facade and route consumers through it.
...
- For history search or transcript analysis, treat `raw_messages`, `raw_token_usage`, `normalized_messages`, `derived_timeline_events`, and `derived_goals` as the canonical pipeline waypoints before inventing new storage. Before adding any new table or column, read `oss/docs/warehouse-layering.md` — it defines which layer (`raw_*` / `normalized_*` / classified `derived_*` / aggregate `derived_*`) new data belongs in and what each layer is allowed to contain.
