...
def _practice_chart(rows: list[tuple[str, str, int]] | None) -> dict[str, Any]:
    ...
    for name, kind, count in rows or []:
        ...
        bucket = (
            "agent"
            if kind in _CHART5_AGENT_KINDS
            else "skill"
            if kind in _CHART5_SKILL_KINDS
            else "other"
        )
        per_name.setdefault(name, {"agent": 0, "skill": 0, "other": 0})[bucket] += count
        total += count
    top = sorted(per_name.items(), key=lambda item: (-sum(item[1].values()), item[0]))[
        :_CHART5_TOP_N
    ]
    return {
...
def aggregate_report_data(  # pylint: disable=too-many-locals
    *,
    ...
    bucket_granularity: Literal["day", "week"] | None = None,
) -> dict[str, Any]:
    ...
    dates = [dt for row in token_rows if (dt := _parse_utc(row.timestamp)) is not None]
    dates.extend(dt for day in session_rows if (dt := _parse_utc(day)) is not None)
    if not dates:
        return _empty_data(state, practice)
    earliest, latest = min(dates), max(dates)
    granularity = bucket_granularity or ("day" if (latest - earliest).days <= 30 else "week")
    ...
    for row in token_rows:
        ...
        if bucket not in threads or value is None:
            continue
        by_model.setdefault(row.model or _MODEL_UNKNOWN_LABEL, dict.fromkeys(buckets, 0.0))[
            bucket
        ] += value
    series = [
        {
    ...
    return {
        ...
        "chart2_line": [
            round(sessions[b] / threads[b], 2) if threads[b] else None for b in buckets
        ],
        "chart3_mode": "cost" if effective_pricing else "tokens",
        "chart3_series": series,
        "chart5": practice,
