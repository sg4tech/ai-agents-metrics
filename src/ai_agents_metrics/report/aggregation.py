...
def _apply_token_pricing(inp: int, cached: int, out: int, model: str | None,
                         pricing: dict[str, dict[str, float | None]] | None) -> float | None:
    if pricing is None:
        return float(inp + cached + out)
    rates = pricing.get(model or "") if model else None
    if not rates:
        return None
    return (
        inp * (rates.get("input_per_million_usd") or 0.0)
        + cached * (rates.get("cached_input_per_million_usd") or 0.0)
        + out * (rates.get("output_per_million_usd") or 0.0)
    ) / 1_000_000
...
def _empty_data(state: dict[str, str], practice: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "chart1_product": [], "chart1_meta": [], "chart1_retro": [],
        "chart2_bar": [], "chart2_line": [], "chart2_source": "warehouse",
        "chart3_mode": "tokens", "chart3_source": "warehouse", "chart3_series": [],
        "chart4_buckets": [], "chart4_values": [], "chart5": practice,
        "ledger_date_from": None, "ledger_date_to": None,
...
def aggregate_report_data(  # pylint: disable=too-many-locals
    *,
    ...
    warehouse_tokens: list[tuple[str, str | None, int, int, int]],
    days: int | None = None,
    ...
    retry_rows = warehouse_retry
    if days is not None:
        cutoff = datetime.now(tz=UTC) - timedelta(days=days)
        token_rows = [row for row in token_rows if (dt := _parse_utc(row[0])) is not None and dt >= cutoff]
        retry_rows = {day: values for day, values in retry_rows.items()
                      if (dt := _parse_utc(day)) is not None and dt >= cutoff}
    dates = [dt for row in token_rows if (dt := _parse_utc(row[0])) is not None]
    dates.extend(dt for day in retry_rows if (dt := _parse_utc(day)) is not None)
    if not dates:
    ...
    retries = dict.fromkeys(buckets, 0)
    for day, values in retry_rows.items():
        if (dt := _parse_utc(day)) is None:
            continue
        bucket = _bucket_key(dt, granularity)
        if bucket in threads:
            threads[bucket] += values.get("threads", 0)
            retries[bucket] += values.get("retry_threads", 0)
    by_model: dict[str, dict[str, float]] = {}
    for timestamp, model, inp, cached, out in token_rows:
        if (dt := _parse_utc(timestamp)) is None:
            continue
        bucket = _bucket_key(dt, granularity)
        value = _apply_token_pricing(inp, cached, out, model, pricing)
        if bucket not in threads or value is None:
            continue
        by_model.setdefault(model or _MODEL_UNKNOWN_LABEL, dict.fromkeys(buckets, 0.0))[bucket] += value
    series = [
        {
            "name": model,
    ...
    retry_threads = sum(retries.values())
    return {
        "granularity": granularity, "buckets": buckets,
        "chart1_product": [threads[b] for b in buckets], "chart1_meta": [0] * len(buckets),
        "chart1_retro": [0] * len(buckets),
        "chart2_bar": [retries[b] for b in buckets],
        "chart2_line": [round(retries[b] / threads[b] * 100, 1) if threads[b] else None for b in buckets],
        "chart2_source": "warehouse", "chart3_mode": "cost" if pricing else "tokens",
        "chart3_source": "warehouse", "chart3_series": series,
        "chart4_buckets": [], "chart4_values": [], "chart5": practice,
        "ledger_date_from": None, "ledger_date_to": None,
        "history_date_from": earliest.strftime("%Y-%m-%d"), "history_date_to": latest.strftime("%Y-%m-%d"),
        "warehouse_state": state,
        "summary": {"total_closed": total_threads, "success_count": None, "success_rate_pct": None,
                    "total_cost_usd": None, "avg_cost_usd": None, "cost_trend": None,
                    "retry_threads": retry_threads,
                    "date_from": earliest.strftime("%Y-%m-%d"), "date_to": latest.strftime("%Y-%m-%d")},
    }
