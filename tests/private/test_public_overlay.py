...
def test_public_overlay_mirror_includes_security_verify_and_rules() -> None:
    makefile_text = (Path(__file__).resolve().parents[2] / "oss" / "Makefile").read_text(
        encoding="utf-8"
    )
    rules_text = (Path(__file__).resolve().parents[2] / "oss" / "config" / "security-rules.toml").read_text(
        encoding="utf-8"
    )
